...
{
    [SerializeField] protected Renderer render = null;
    [SerializeField] private MaterialPropertyBlock propBlock = null;
    ...
    public void UpdateProperty((string key, Vector4[] valueArray)[] shadersData)
    {
        GetData(render, ref propBlock); foreach (var shaderData in shadersData)
        {
            propBlock.SetVectorArray(shaderData.key, shaderData.valueArray);
        }
        SendData();
    }
    public void UpdateProperty((string key, float[] valueArray)[] shadersData)
    {
        GetData(render, ref propBlock); foreach (var shaderData in shadersData)
        {
            propBlock.SetFloatArray(shaderData.key, shaderData.valueArray);
        }
        SendData();
    }
